...
public class Parser {
    ...
    private JClassDeclaration classDeclaration(ArrayList<String> mods) {
        ...
        ArrayList<Type> implementz = new ArrayList<>();
        Type superClass;
        if (have(EXTENDS)) {
        ...
        if(have(IMPLEMENTS)){
           implementz.add(qualifiedIdentifier());
           while (have(COMMA)){
               implementz.add(qualifiedIdentifier());
           }
        }
        return new JClassDeclaration(line, mods, name, superClass, classBody(), implementz);
    }
    ...
    private JInterfaceDeclaration interfaceDeclaration(ArrayList<String> mods) {
        ...
        mustBe(IDENTIFIER);
        String name = scanner.previousToken().image();
        ArrayList<Type> superClass = new ArrayList<>();
        if(have(EXTENDS)){
    ...
    private JMember memberDecl(ArrayList<String> mods) {
        ...
        if (seeIdentLParen()) {
            ...
            if (have(VOID)) {
                ...
                memberDecl = new JMethodDeclaration(line, mods, name, type, params, body, null);
            } else {
                type = type();
                if (seeIdentLParen()) {
                    // Non void method
     ...
     *
     * <pre>
     * interfaceMemberDecl ::= ( VOID | type ) IDENTIFIER formalParameters SEMI | type variableDeclarators SEMI
     * </pre>
     *
    ...
    private JMember interfaceMemberDecl(ArrayList<String> mods){
        ...
        if(have(VOID)){
            ...
            ArrayList<JFormalParameter> params = formalParameters();
            mustBe(SEMI);
            imd = new JMethodDeclaration(line, mods, name, type, params, null, null);
        }else{
            type = type();
            if (seeIdentLParen()) {
                mustBe(IDENTIFIER);
                ...
                imd = new JFieldDeclaration(line, mods, variableDeclarators(type));
                mustBe(SEMI);
            }
        }
        return imd;
...
//        } else if (have(TRY)){
//            JBlock tryBlock;
//            ArrayList<JFormalParameter> formPar = new ArrayList<>();
//            ArrayList<JBlock> catchBlock = new ArrayList<>();
//            JBlock finalBlock = null;
//            tryBlock = block();
//            while(have(CATCH)){
//                mustBe(LPAREN);
//                formPar.add(formalParameter());
//                mustBe(RPAREN);
//                catchBlock.add(block());
//            }
//            if(have(FINALLY)){
//                if(see(CATCH)){
//                    reportParserError("FINALLY BLOCK INVALID LOCATION");
//                }else {
//                    finalBlock = block();
//                }
//            }
//            return new JTryStatement(line, tryBlock, formPar, catchBlock, finalBlock);
        } else if (have(FOR)){
            ...
            mustBe(RPAREN);
            statement = statement();
            return new JForStatement(line, forInit, expr, forUpdate, statement);
        } else if (have(WHILE)) {
            JExpression test = parExpression();
            JStatement statement = statement();
            ...
            ArrayList<JSwitchStatement> switchBlockStatementGroup = new ArrayList<>();
            ...
            mustBe(RCURLY);
            return new JSwitchStatement(line, parExpression, switchBlockStatementGroup, null,
                    null, false);
        }
        else {
    ...
    /**
     * Parses and returns a list of initialized For-Statement condition parameters.
     *
     * <pre>
    ...
    private ArrayList<JStatement> forInit(int line) {
        ArrayList<JStatement> inits = new ArrayList<JStatement>();
        if (seeBasicType() || seeReferenceType()) {
            ArrayList<JVariableDeclarator> vdecls = variableDeclarators(type());
            inits.add(new JVariableDeclaration(line, vdecls));
        } else {
            inits.add(statementExpression());
            while(have(COMMA)) {
                inits.add(statementExpression());
            }
        }
    ...
    /**
     * Parses and returns a list of updated For-Statement condition parameters.
     *
     * <pre>
    ...
    /**
     * Parses and returns a list of cases and related blocks.
     *
     * <pre>
     *   switchBlockStatementGroup ::= switchLabel { switchLabel } { blockStatement }
     * </pre>
     *
     * @return an object of Cases and related Block of code .
     */
    private JSwitchStatement switchBlockStatementGroup() {
        int line = scanner.token().line();
        ...
        return new JSwitchStatement(line, null, null, switchLabels, blockStatements, true);
    }
     ...
     *
     * <pre>
     *   switchLabel ::= CASE expression COLON | DEFLT COLON
     * </pre>
     *
     * @return an AST for a switch label expression.
     */
     ...
     *
     * <pre>
     *   basicType ::= BOOLEAN | CHAR | INT | LONG | BOUBLE
     * </pre>
     *
     * @return a basic type.
     */
    ...
    private JExpression conditionalExpression() {
        ...
        JExpression lhs = conditionalOrExpression();
            if (have(QUESTION)){
                JExpression t1 = expression();
                mustBe(COLON);
                JExpression t2 = conditionalExpression();
                return new JConditionalExpression(line, lhs, t1, t2);
            } else {
                return lhs;
            }
    }
    ...
    /**
     * Parses a bitwise XOR expression and returns an AST for it.
     */
    ...
    /**
     * Parses a bitwise AND expression and returns an AST for it.
     */
    ...
    /**
     * Parses a bitwise Arithmetic and Logical Shift expressions and returns an AST for it.
     */
