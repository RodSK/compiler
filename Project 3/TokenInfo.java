...
enum TokenKind {
    ...
    EXTENDS("extends"), FINALLY("finally"), FOR("for"), IF("if"), IMPORT("import"),
    IMPLEMENTS("implements"), INTERFACE("interface"), LONG("long"), INSTANCEOF("instanceof"), INT("int"),
    NEW("new"), PACKAGE("package"), PRIVATE("private"), PROTECTED("protected"),
    PUBLIC("public"), RETURN("return"), STATIC("static"), SUPER("super"), SWITCH("switch"),
    TRY("try"), THIS("this"), THROW("throw"), THROWS("throws"),
    VOID("void"), WHILE("while"),

    // Operators.
    ASSIGN("="), DEC("--"), EQUAL("=="), GT(">"), INC("++"), LAND("&&"), LE("<="), LNOT("!"),
    MINUS("-"), PLUS("+"), PLUS_ASSIGN("+="), STAR("*"), DIV("/"), REM("%"), NOT("~"),
    OR("|"), XOR("^"), AND("&"), ALSHIFT("<<"), ARSHIFT(">>"), LRSHIFT(">>>"), QUESTION("?"),
    COLON(":"), NOT_EQUAL("!="), DIV_ASSIGN("/="), MINUS_ASSIGN("-="), STAR_ASSIGN("*="),
    ...
    STRING_LITERAL("<STRING_LITERAL>"), LONG_LITERAL("<LONG_LITERAL>"), DOUBLE_LITERAL("<DOUBLE_LITERAL>");
