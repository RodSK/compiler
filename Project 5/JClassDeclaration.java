...
class JClassDeclaration extends JAST implements JTypeDecl {
    ...
    private ArrayList<Type> implementz;
    ...
    public void preAnalyze(Context context) {
        ...
        for (int i = 0; i < implementz.size(); i++) {
            implementz.set(i, implementz.get(i).resolve(this.context));
            if (!implementz.get(i).isInterface()) {
                JAST.compilationUnit.reportSemanticError(line, "Not Interface: %s",
                        implementz.get(i).toString());
            }
        }
        ...
        ArrayList<String> supreInterfaces = new ArrayList<String>();
        for (int i = 0; i < implementz.size(); i++) {
            supreInterfaces.add(implementz.get(i).jvmName());
        }

        partial.addClass(mods, qualifiedName, superType.jvmName(), supreInterfaces, false);
    ...
    public ArrayList<Type> implementz() {return implementz; }
    ...
    public void codegen(CLEmitter output) {
        ...
        output.addClass(mods, qualifiedName, superType.jvmName(), supreInterfaces, false);
